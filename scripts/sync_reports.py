...
def parse_meta(html_text, sector_ids):
    ...
    # 섹터 id 형식 검증(소문자 영문/숫자/_ , 2~16자). 형식이 맞으면 신규 섹터라도 통과 →
    # upsert에서 목록에 없으면 자동 생성. 형식이 틀리면 None(미분류)으로.
    sector = sector_raw if isinstance(sector_raw, str) and re.fullmatch(r"[a-z][a-z0-9_]{1,15}", sector_raw) else None
    ...
    return {
        ...
        "sector_factor": data.get("sector_factor"),
...
def upsert(master, code, name, fname, mdate, meta):
    ...
    existing_ids = {s["id"] for s in master["sectors"]}
    if classified and sector not in existing_ids:
        # 신규 섹터 자동 생성 — 이름·팩터는 메타에서, 없으면 id 기반 기본값
        new_sec = {
            "id": sector,
            "name": m.get("sector_name") or sector.upper(),
            "factor": m.get("sector_factor") or "신규 섹터 — 팩터 설명은 편집 페이지에서 보완",
        }
        master["sectors"].append(new_sec)
        print(f"  ★ 새 섹터 자동 생성: {sector} ({new_sec['name']}) — 편집 페이지에서 확인 권장")
